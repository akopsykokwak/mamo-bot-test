    ...
    try {
      ...
      const searchCharacterId = async () => {
        try {
          ...
          console.log(error)
          if (error.error && error.error.Message === 'Lodestone is currently down for maintenance.') return message.channel.send(`🚫 Le Lodestone est en maintenance actuellement, veuillez réessayer plus tard.`)
          else message.channel.send(`🚫 Aucun personnage n'a été trouvé avec ces informations.`)
        }
      }
      ...
      const searchCharacter = async id => {
        try {
          ...
          jobsData.map(async data => {
            try {
              await axios.get(`https://xivapi.com/classjob/${data.ClassID}?private_key=${process.env.XIV_KEY}`).then(res => {
                const category = res.data.ClassJobCategory.Name_fr;
                db.collection('characters').doc(id.toString()).collection('classJobs').doc(data.UnlockedState.Name).set({ data, Category: category });
              })
            } catch (error) {
              console.log(error)
            }
          ...
          searchCollection(id)
          if(charaData.FreeCompanyId) searchFreeCompany(charaData.FreeCompanyId, id);
        } catch (error) {
          console.log(error)
          if (error.error.Message === 'Lodestone is currently down for maintenance.') return message.channel.send(`🚫 Le Lodestone est en maintenance actuellement, veuillez réessayer plus tard.`)
          else message.send.channel(`🚫 Aucun personnage n'a été trouvé avec ces informations.`)
        }
      }
      ...
      const searchFreeCompany = async (fCId, charaId) => {
        try {
          ...
          console.log(error)
          if (error.error.Message === 'Lodestone is currently down for maintenance.') return message.channel.send(`🚫 Le Lodestone est en maintenance actuellement, veuillez réessayer plus tard.`)
          else message.channel.send(`🚫 Aucune Compagnie Libre n'a été trouvé avec ces informations.`)
        }
      }
